...
export default function Overview({ api, selectedRestaurant }) {
  ...
  const [itemSort, setItemSort]   = useState('qty')  // 'qty' | 'revenue'
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setOrders(raw.sort((a, b) => new Date(b.orderedAt) - new Date(a.orderedAt)))
  ...
  const realOrders   = orders.filter(o => o.isRealOrder)
  const totalRevenue = realOrders.reduce((sum, o) => sum + (o.pricing?.total || 0), 0)
  const todayRevenue = todayOrders.filter(o => o.isRealOrder).reduce((sum, o) => sum + (o.pricing?.total || 0), 0)
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Placed"           value={statusCounts['placed'] || 0} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div key={item.name}>
                ...
                <div className="h-1.5 bg-bg rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(item[itemSort] / maxVal) * 100}%` }}
                  />
                </div>
              </div>
            ))}
        ...
        {/* Recent Orders */}
        <div>
          <h2 className="text-sm font-semibold text-text-secondary mb-3">Recent Orders</h2>
            ...
            {orders.slice(0, 8).map(o => (
              <div key={o._id} className="bg-bg-card border border-border rounded-lg px-4 py-3 flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-text-muted text-xs">{new Date(o.orderedAt).toLocaleDateString()}</span>
